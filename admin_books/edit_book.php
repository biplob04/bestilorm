	...
	<div id="container">
		...
		<div id="content">
			<div id="nota-books" class="warning-box">
				...
				<?php else: ?>
					<p>Não existe um livro com ID #<?php echo $id; ?></p>
				<?php endif; ?>
			</div>
			...
			<?php if($book): ?>
				<div id="insert-book">
					<div id="insertB-title">
						<i class="material-icons noselect">edit</i>
						<p>Editar o livro "<?php echo $book->nomeLivro; ?>"</p>
					</div>
					<div id="insertB-form">
						<form action="../assets/edit_book.php" method="post" accept-charset="utf-8">
							<input type="text" name="bookId" value="<?php echo $book->idLivro; ?>" style="display:none;">
							<div class="inputB">
								<label for="bookInput" id="bookInput">Nome</label>
								<input type="text" name="bookName" size="25" onfocus="inputInteraction('#292f36', 'bookInput');" onfocusout="if(this.value == '') { inputInteraction('red', 'bookInput'); } else { inputInteraction('#49C5B6', 'bookInput'); }" value="<?php echo $book->nomeLivro; ?>" required>
							</div>
							<div class="inputB" style="width:20%">
								<label for="isbnInput" id="isbnInput">ISBN</label>
								<input type="text" name="bookIsbn" maxlength="12" size="20" onfocus="inputInteraction('#292f36', 'isbnInput');" onfocusout="if(this.value == '') { inputInteraction('red', 'isbnInput'); } else { inputInteraction('#49C5B6', 'isbnInput'); }" value="<?php echo $book->isbnLivro; ?>" required>
							</div>
							<div class="inputB" style="width:27%">
								...
								<select name="temaSelect" style="width:130px">
									<?php foreach($genres as $genre): ?>
										<option value="<?php echo $genre->idTema; ?>" <?php if ($book->tema == $genre->tema) {echo "selected";} ?>><?php echo $genre->tema; ?></option>
									<?php endforeach; ?>
								</select>
							...
							<div class="inputB" style="width:31%">
								...
								<select name="autSelect" style="width:180px">
									<?php foreach($authors as $author): ?>
										<option value="<?php echo $author->idAutor; ?>" <?php if ($book->nomeAutor == $author->nomeAutor) {echo "selected";} ?>><?php echo $author->nomeAutor; ?></option>
									<?php endforeach; ?>
								</select>
							...
							<div class="inputB" style="width:33%">
								...
								<select name="editSelect" style="width:200px">
									<?php foreach($publishers as $publisher): ?>
										<option value="<?php echo $publisher->idEditora; ?>" <?php if ($book->nomeEditora == $publisher->nomeEditora) {echo "selected";} ?>><?php echo $publisher->nomeEditora; ?></option>
									<?php endforeach; ?>
								</select>
							...
							<div class="inputB" style="width:20%">
								...
								<input type="text" name="pubDate" maxlength="4" size="10" onfocus="inputInteraction('#292f36', 'dateInput');" onfocusout="if(this.value == '') { inputInteraction('red', 'dateInput'); } else { inputInteraction('#49C5B6', 'dateInput'); }" value="<?php echo $book->dataPublicLivro; ?>" required>
							</div>
							<div class="inputB" style="width:20%">
								<label for="stockInput" id="stockInput">Stock presente</label>
								<input type="text" name="stock" maxlength="5" size="10" onfocus="inputInteraction('#292f36', 'stockInput');" onfocusout="if(this.value == '') { inputInteraction('red', 'stockInput'); } else { inputInteraction('#49C5B6', 'stockInput'); }" value="<?php echo $book->stockLivro; ?>" required>
							</div>
							<div class="inputB" style="width:18%">
								<label for="priceInput" id="priceInput">Preço</label>
								<input type="number" name="price" size="10" min="0.00" max="100000.00" step="0.01" onfocus="inputInteraction('#292f36', 'priceInput');" onfocusout="if(this.value == '') { inputInteraction('red', 'priceInput'); } else { inputInteraction('#49C5B6', 'priceInput'); }" value="<?php echo $book->precoLivro; ?>" required>
							</div>
							<div class="inputB" style="width:40%;">
								<label for="descInput" id="descInput">Descrição</label>
								<textarea name="descBook" maxlength="500" onfocus="inputInteraction('#292f36', 'descInput');" onfocusout="if(this.value == '') { inputInteraction('red', 'descInput'); } else { inputInteraction('#49C5B6', 'descInput'); }"><?php echo $book->descricaoLivro; ?></textarea>
							</div>
							<div class="inputB" style="width:28%;height:85px">
								<label for="imageInput" id="imageInput">Caminho para a imagem</label>
								<input type="text" name="image" maxlength="50" size="30" onfocus="inputInteraction('#292f36', 'imageInput');" onfocusout="if(this.value == '') { inputInteraction('red', 'imageInput'); } else { inputInteraction('#49C5B6', 'imageInput'); }" value="<?php echo $book->imagemLivro; ?>" required>
							</div>
							<button type="submit" name="editarBook" value="s">Editar</button>
						</form>
					</div>
				</div>
			<?php endif; ?>
